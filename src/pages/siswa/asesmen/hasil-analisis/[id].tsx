...
import { FaArrowLeft, FaChartLine, FaClock, FaLightbulb, FaPoll, FaTrophy, FaCheckCircle, FaExclamationCircle, FaHourglassEnd, FaGraduationCap } from 'react-icons/fa';
...
export default function HasilAnalisisAsesmenSiswa() {
  ...
  useEffect(() => {
    // Setelah asesmen selesai, halaman ini muat ringkasan attempt + analisis TP siswa.
    const loadData = async () => {
      ...
      try {
        ...
        const fetchAnalisis = async () => {
          const response = await fetch(`/api/asesmen/analisis?id_asesmen=${asesmenId}&id_siswa=${session.id_siswa}`);
        ...
        if (!data.pending_validation && (!data.analysis || data.analysis.length === 0)) {
          // Kalau analisis belum tersedia, generate fallback dulu lalu fetch ulang.
          await fetch('/api/asesmen/generate-analisis-fallback', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id_asesmen: asesmenId, id_siswa: session.id_siswa }),
          });
        ...
        // Fetch jawaban data
        try {
          const jawabanResponse = await fetch(`/api/asesmen/jawaban-siswa?id_siswa=${session.id_siswa}&id_asesmen=${asesmenId}`);
          if (jawabanResponse.ok) {
            const jawabanData: JawabanResponse = await jawabanResponse.json();
            setJawabanList(jawabanData.jawaban);
          } else {
            console.error('Gagal mengambil data jawaban');
          }
        } catch (error) {
  ...
  const calculateTotalScore = () => {
    // Total skor mengikuti hasil validasi terbaru per soal.
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <p className="text-2xl font-bold flex items-center gap-2">
              ...
              <span className="text-white">{siswaSession.nama_siswa}</span>
            </p>
          </div>
          ...
          {/* Jawaban & Validasi Section */}
          <div className="mt-12">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
              <FaCheckCircle className="text-white" />
              Jawaban & Validasi
            </h2>
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-gray-800/50 to-gray-900/50 p-6 md:p-8 overflow-x-auto">
              <JawabanSiswaTable
